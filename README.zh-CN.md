...
> **开发状态：** 数据契约、加固的哈希、太阳系预设、星系视图、近焦 2D 和可运行的 demo 已在仓库中。近焦 3D 以预览子集发布（含无接缝光晕的恒星着色器、轨道布局、恒星/行星拾取、纯视觉的小行星带与卫星）；bloom 后处理、镜头跟随导航、带/卫星详情事件和逐行星表面着色器仍在上游路线图上。
...
| `src/near-focus-3d` | 预览 | React Three Fiber 星系视图，带导航与恒星/行星详情事件；带与卫星目前纯视觉。 |
